...
async def upload_price_file(
    ...
    2. читаем тело с проверкой лимита 10 МБ;
    3. кладём во временную папку ПОД ИСХОДНЫМ ИМЕНЕМ файла,
       чтобы в отчёте и в price_uploads сохранилось реальное имя (напр. supplier_a.csv);
    4. вызываем import_price_file и возвращаем отчёт;
    5. временная папка удаляется автоматически при выходе из with.
    """
    ext = _validate_suffix(file.filename)

    # Читаем всё тело в память. Прайсы небольшие (лимит 10 МБ), это безопасно.
